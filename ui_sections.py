...
def _load_jp_font(size: int) -> ImageFont.FreeTypeFont | ImageFont.ImageFont:
    """Load a Japanese-capable font for Pillow rendering (Windows-first).
    This is used ONLY for the '資料作成用データ' PNG export to avoid garbled text.
    """
    candidates = [
...
def _load_jp_bold_font(size: int) -> ImageFont.FreeTypeFont | ImageFont.ImageFont:
    candidates = [
    ...
    for p in candidates:
        try:
            ...
            continue
    return _load_jp_font(size)


def _jp_font_path(prefer_bold: bool = False) -> Optional[str]:
    candidates = [
        r"C:\Windows\Fonts\meiryo.ttc",
...
def _draw_shadowed_round_rect(
    base: Image.Image,
    xy: tuple[int, int, int, int],
    radius: int,
    fill: str,
    shadow_offset: tuple[int, int] = (0, 8),
    shadow_radius: int = 14,
    shadow_alpha: int = 20,
) -> None:
    ...
    sx0, sy0, sx1, sy1 = xy
    ox, oy = shadow_offset
    shadow_draw.rounded_rectangle((sx0 + ox, sy0 + oy, sx1 + ox, sy1 + oy), radius=radius, fill=(0, 0, 0, shadow_alpha))
...
def _format_short_jp_period(start_date: Optional[date], end_date: Optional[date], fallback_dates: pd.Series) -> str:
    if start_date is None or end_date is None:
        dates = pd.to_datetime(fallback_dates, errors="coerce").dropna()
        if dates.empty:
            return ""
        start_date = dates.min().date()
        end_date = dates.max().date()
    return f"（{start_date.year % 100:02d}年{start_date.month}月～{end_date.year % 100:02d}年{end_date.month}月）"


def _shadow_card_monthly_average_png(
    ...
    y_axis_title_scale: float = 1.0,
    y_axis_tick_scale: float = 1.0,
    legend_scale: float = 1.0,
) -> bytes:
    if not MATPLOTLIB_AVAILABLE:
        return _fallback_multi_line_png(series, "各月デマンド平均グラフ", y_axis_label or display_unit)

    from matplotlib import font_manager
    card_w, card_h = 1580, 564
    pad_x, pad_y = 36, 28
    chart_w, chart_h = card_w - pad_x * 2, card_h - pad_y * 2

    canvas = Image.new("RGBA", (card_w + 90, card_h + 88), "#FFFFFF")
    card_xy = (45, 22, 45 + card_w, 22 + card_h)
    _draw_shadowed_round_rect(
        canvas,
        card_xy,
        radius=30,
        fill="#FFFFFF",
    ...
    if finite_vals:
        data_min = float(min(finite_vals))
        data_max = float(max(finite_vals))
        span = max(data_max - data_min, 1.0)
        step = _nice_step(span / 6.0)
        ymin = max(0.0, np.floor((data_min - span * 0.06) / step) * step)
        ymax = data_max + span * 0.08
        if ymax <= ymin:
            ymax = ymin + step
        y_tick_max = np.floor(ymax / step) * step
        if y_tick_max < data_max and (data_max - y_tick_max) < step * 0.35:
            pass
        elif y_tick_max < data_max:
            y_tick_max += step
        y_ticks = np.arange(ymin, y_tick_max + step * 0.5, step)
    else:
        ymin, ymax = 0.0, 10.0
        y_ticks = np.arange(ymin, ymax + 1.0, 2.0)

    if y_range is not None:
        ymin, ymax = float(y_range[0]), float(y_range[1])
        step = _nice_step((ymax - ymin) / 6.0)
        tick_start = np.ceil(ymin / step) * step
        tick_end = np.floor(ymax / step) * step
        y_ticks = np.arange(tick_start, tick_end + step * 0.5, step)
        if y_ticks.size < 2:
            y_ticks = np.array([ymin, ymax])

    font_path = _jp_font_path()
    ...
    x = list(range(len(slot_labels)))
    for name, vals in series.items():
        is_total = name == "全体"
        ax.plot(
            x[: len(vals)],
            vals,
            color=colors.get(name, "#636EFA"),
            linewidth=1.9 if is_total else 1.45,
            linestyle="--" if is_total else "-",
            label=name,
            solid_capstyle="round",
        )

    ax.set_xlim(-0.5, max(len(slot_labels) - 0.5, 1))
    ax.set_ylim(ymin, ymax)
    ax.set_yticks(y_ticks)
    ax.grid(axis="y", color="#E2E5EA", linewidth=1.0, linestyle=(0, (1, 3)))
    ax.grid(axis="x", visible=False)
    for spine in ax.spines.values():
        spine.set_visible(False)

    tick_idx = [idx for idx, label in enumerate(slot_labels) if str(label).endswith(":00")]
    ax.set_xticks(tick_idx)
    time_font_prop = font_manager.FontProperties(family="Arial", size=5.4)
    ax.set_xticklabels([slot_labels[i] for i in tick_idx], color="#5F666D", fontproperties=time_font_prop, ha="center")
    ax.tick_params(axis="x", length=0, pad=10)
    ax.tick_params(axis="y", colors="#6E737A", labelsize=10.5 * y_axis_tick_scale, length=0, pad=8)
    for label in ax.get_yticklabels():
        if font_prop:
            label.set_fontproperties(font_prop)
    ax.set_ylabel(
        y_axis_label or f"平均[{display_unit}]",
        fontsize=12 * y_axis_title_scale,
        color="#60656B",
        labelpad=10,
    ...
    legend = ax.legend(
        ...
        labelspacing=0.52,
        borderaxespad=0,
        prop=font_manager.FontProperties(fname=font_path, size=8.2 * legend_scale)
        if font_path
        else font_manager.FontProperties(size=8.2 * legend_scale),
    )
    for line in legend.get_lines():
        line.set_linewidth(1.9)

    fig.subplots_adjust(left=0.085, right=0.895, top=0.95, bottom=0.16)
    chart_buf = BytesIO()
    fig.savefig(chart_buf, format="png", dpi=dpi, transparent=True)
    plt.close(fig)
    chart_buf.seek(0)
    chart = Image.open(chart_buf).convert("RGBA")
    canvas.alpha_composite(chart, (card_xy[0] + pad_x, card_xy[1] + pad_y))
...
def _cell_style_bg_only(value: float, vmin: float, vcenter: float, vmax: float) -> str:
    """3色（青→白→赤）の背景のみ。テキスト色は常に黒。
    Excelの見た目（スクリーンショット）に合わせて端色を固定。
    """
    blue = np.array([97, 203, 243], dtype=float)  # #61CBF3
    white = np.array([255, 255, 255], dtype=float)
    red = np.array([255, 124, 128], dtype=float)  # #FF7C80

    if value is None or not np.isfinite(value) or vmax <= vmin:
        r, g, b = white.astype(int)
        return f"background-color: rgb({int(r)},{int(g)},{int(b)}); color: #000000"

    if value <= vcenter:
        den = max(vcenter - vmin, 1e-12)
        t = float(np.clip((value - vmin) / den, 0.0, 1.0))
        rgb = blue * (1.0 - t) + white * t
    else:
        den = max(vmax - vcenter, 1e-12)
        t = float(np.clip((value - vcenter) / den, 0.0, 1.0))
        rgb = white * (1.0 - t) + red * t
...
def _styled_material_data_table(df: pd.DataFrame, heat_cols: list[str], sum_row_label: str) -> pd.io.formats.style.Styler:
    """資料作成用データ向け：指定列だけ平均中心の3色背景。
    - 合計行（sum_row_label）は色付け対象から除外
    """
    work = df.copy()
    base = work[work[work.columns[0]] != sum_row_label].copy()

    numeric = base[heat_cols].apply(pd.to_numeric, errors="coerce")
    stats: dict[str, tuple[float, float, float]] = {}
    for c in heat_cols:
        col = pd.to_numeric(numeric[c], errors="coerce")
        finite = col[np.isfinite(col.to_numpy(dtype=float))]
        if finite.empty:
            stats[c] = (0.0, 0.0, 0.0)
        else:
            stats[c] = (float(finite.min()), float(finite.mean()), float(finite.max()))
    ...
    for c in work.columns:
        ...
        if "利用率" in cstr:
            ...
            fmt[c] = "{}"
        elif "(kW" in cstr or "(kWh" in cstr or "(" in cstr:
            fmt[c] = "{:,.0f}"
        else:
            fmt[c] = "{}"
    ...
    def _apply_series(s: pd.Series, col_name: str) -> list[str]:
        out: list[str] = []
        for i, v in enumerate(s.tolist()):
            row_label = work.iloc[i, 0]
            if str(row_label) == str(sum_row_label):
                out.append("color: #000000")
                continue
            if col_name not in stats:
                out.append("color: #000000")
                continue
            vmin, vcenter, vmax = stats[col_name]
            try:
                fv = float(v)
            except Exception:
                out.append("color: #000000")
                continue
            out.append(_cell_style_bg_only(fv, vmin, vcenter, vmax))
        return out

    for c in heat_cols:
        styled = styled.apply(lambda s, col=c: _apply_series(s, col), axis=0, subset=pd.IndexSlice[:, [c]])
...
def render_annual_monthly_average(
    ...
    # 表示単位（このセクションは30分スロット値なので kW <-> kWh 変換が自然）
    display_unit = st.selectbox("表示単位", ["kW", "kWh"], index=0, key="annual_monthly_avg_unit")
    factor = 0.5 if display_unit == "kWh" else 1.0
    ...
    def _month_color(month: int) -> str:
        ...
        """
        # 冬（青系）: 11,12,1,2,3
        winter = {11: "#08306B", 12: "#08519C", 1: "#2171B5", 2: "#4292C6", 3: "#6BAED6"}
        # 春（黄緑系）: 4,5
        spring = {4: "#7CB342", 5: "#C0CA33"}
        # 夏（赤系）: 6,7,8
        summer = {6: "#B71C1C", 7: "#E53935", 8: "#FF7043"}
        # 秋（空色系）: 9,10
        autumn = {9: "#00ACC1", 10: "#26C6DA"}
        mmap = {}
        mmap.update(winter)
        mmap.update(spring)
        mmap.update(summer)
        mmap.update(autumn)
        return mmap.get(int(month), "#636EFA")

    col1, col2 = st.columns(2)
    with col1:
        annual_df = result.annual_avg_curve.copy()
        annual_df["avg_disp"] = pd.to_numeric(annual_df["avg_kw"], errors="coerce").fillna(0.0) * factor
        annual_show = annual_df.rename(columns={"slot_label": "時刻"}).assign(**{f"平均({display_unit})": annual_df["avg_disp"]})
        _render_line(annual_show[["時刻", f"平均({display_unit})"]], "時刻", f"平均({display_unit})", "年間デマンド平均", "年間デマンド平均")

    with col2:
        one = result.monthly_avg_curve[result.monthly_avg_curve["month"] == selected_month].copy()
        one["avg_disp"] = pd.to_numeric(one["avg_kw"], errors="coerce").fillna(0.0) * factor
        one_show = one.rename(columns={"slot_label": "時刻"}).assign(**{f"平均({display_unit})": one["avg_disp"]})
        _render_line(one_show[["時刻", f"平均({display_unit})"]], "時刻", f"平均({display_unit})", f"月別デマンド平均（{int(selected_month)}月）", f"月別平均_{int(selected_month)}月")
    ...
    for month, grp in month_curve.groupby("month"):
        m = int(month)
        label = f"{m}月"
        y_vals = pd.to_numeric(grp["avg_kw"], errors="coerce").fillna(0.0) * factor
        lines_for_png[label] = y_vals.tolist()
        card_colors[label] = _month_color(m)
        fig.add_trace(
            go.Scatter(
                x=grp["slot_label"],
                y=y_vals,
                mode="lines",
                name=label,
                line=dict(color=_month_color(m), width=_line_width(), dash="solid"),
                hovertemplate=f"区分: %{{fullData.name}}<br>時刻: %{{x}}<br>平均: %{{y:.2f}} {display_unit}<extra></extra>",
            )
        )

    annual = result.annual_avg_curve.sort_values("slot").copy()
    annual_y = pd.to_numeric(annual["avg_kw"], errors="coerce").fillna(0.0) * factor
    fig.add_trace(
        go.Scatter(
            x=annual["slot_label"],
            y=annual_y,
            mode="lines",
            name="全体",
            line=dict(color="#111111", width=max(_line_width() + 0.5, 3.0), dash="dash"),
            hovertemplate=f"区分: 全体<br>時刻: %{{x}}<br>平均: %{{y:.2f}} {display_unit}<extra></extra>",
        )
    )
    lines_for_png["全体"] = annual_y.tolist()
    card_colors["全体"] = "#111111"

    y_axis_mode = st.radio(
        "Y軸範囲",
        ["自動", "手動"],
        horizontal=True,
        key="monthly_overlay_y_axis_mode",
    )
    manual_y_range: Optional[list[float]] = None
    if y_axis_mode == "手動":
        observed_y_max = max(
            [value for values in lines_for_png.values() for value in values],
            default=0.0,
        )
        suggested_y_max = max(float(np.ceil(observed_y_max * 1.05)), 1.0)
        y_min_col, y_max_col = st.columns(2)
        with y_min_col:
            y_axis_min = st.number_input(
                f"Y軸の最小値 ({display_unit})",
                value=0.0,
                step=1.0,
                key=f"monthly_overlay_y_axis_min_{display_unit}",
            )
        with y_max_col:
            y_axis_max = st.number_input(
                f"Y軸の最大値 ({display_unit})",
                value=suggested_y_max,
                step=1.0,
                key=f"monthly_overlay_y_axis_max_{display_unit}",
            )
        if y_axis_min < y_axis_max:
            manual_y_range = [float(y_axis_min), float(y_axis_max)]
        else:
            st.warning("Y軸の最大値は最小値より大きい値を入力してください。")

    title_size_col, tick_size_col, legend_size_col = st.columns(3)
    with title_size_col:
        y_axis_title_scale_percent = st.slider(
            "Y軸タイトルの大きさ (%)",
            min_value=50,
            max_value=200,
            value=100,
            step=10,
            key="monthly_overlay_y_axis_title_scale",
        )
    with tick_size_col:
        y_axis_tick_scale_percent = st.slider(
            "Y軸の数字の大きさ (%)",
            min_value=50,
            max_value=200,
            value=100,
            step=10,
            key="monthly_overlay_y_axis_tick_scale",
        )
    with legend_size_col:
        legend_scale_percent = st.slider(
            "凡例の大きさ (%)",
            min_value=50,
            max_value=200,
            value=100,
            step=10,
            key="monthly_overlay_legend_scale",
        )
    y_axis_title_scale = y_axis_title_scale_percent / 100.0
    y_axis_tick_scale = y_axis_tick_scale_percent / 100.0
    legend_scale = legend_scale_percent / 100.0

    fig.update_layout(
        title="1～12月デマンド平均",
        xaxis_title="時刻",
        yaxis_title=f"平均({display_unit})",
        yaxis_title_font_size=14 * y_axis_title_scale,
        yaxis_tickfont_size=12 * y_axis_tick_scale,
        legend_font_size=12 * legend_scale,
        margin=dict(l=40, r=20, t=50, b=50),
    )
    if manual_y_range is not None:
        fig.update_yaxes(range=manual_y_range)
    st.plotly_chart(fig, use_container_width=True)
    ...
    csv_wide = csv_wide.apply(pd.to_numeric, errors="coerce").fillna(0.0) * factor
    csv_wide["全体"] = annual_y.set_axis(annual["slot_label"]).reindex(csv_wide.index)
    csv_out = csv_wide.reset_index().rename(columns={"slot_label": "時刻"})

    fallback = _fallback_multi_line_png(lines_for_png, "1～12月デマンド平均", display_unit)
    card_png = _shadow_card_monthly_average_png(
        slot_labels=annual["slot_label"].astype(str).tolist(),
        series=lines_for_png,
        colors=card_colors,
        display_unit=display_unit,
        y_range=manual_y_range,
        y_axis_title_scale=y_axis_title_scale,
        y_axis_tick_scale=y_axis_tick_scale,
    ...
    with d2:
        st.download_button(
            "影付きグラフPNGダウンロード",
            data=card_png,
            file_name="各月デマンド平均グラフ_影付き.png",
            mime="image/png",
            key="png_monthly_overlay_shadow_card",
        )
    with d3:
        _download_csv(csv_out, "CSVダウンロード", "1～12月デマンド平均.csv", "csv_monthly_overlay")


def render_monthly_slot_peak(result: AnalysisResult) -> None:
    st.subheader("グラフ（時間帯別ピーク）")

    data = result.target_data.copy()
    data["kw"] = pd.to_numeric(data["kw"], errors="coerce")
    data = data.dropna(subset=["kw"])
    if data.empty:
        st.info("時間帯別ピークを計算できるデータがありません。")
        return

    peak_curve = (
        data.groupby(["month", "slot", "slot_label"], as_index=False)["kw"]
        .max()
        .rename(columns={"kw": "max_kw"})
        .sort_values(["month", "slot"])
    )
    slot_order = data[["slot", "slot_label"]].drop_duplicates().sort_values("slot")
    months = sorted(peak_curve["month"].dropna().unique().tolist())
    if not months:
        st.info("月別データがありません。")
        return

    st.caption("各月・各時間帯で記録された最大デマンド(kW)を表示します。")
    st.markdown("**1～12月 時間帯別最大デマンド**")
    ...
    def _month_color(month: int) -> str:
        ...
        color_map.update(winter)
        color_map.update(spring)
        color_map.update(summer)
        color_map.update(autumn)
        return color_map.get(int(month), "#636EFA")

    fig = go.Figure()
    lines_for_png: Dict[str, list[float]] = {}
    card_colors: Dict[str, str] = {}
    for month, grp in peak_curve.groupby("month"):
        m = int(month)
        label = f"{m}月"
        y_vals = pd.to_numeric(grp["max_kw"], errors="coerce").fillna(0.0)
        lines_for_png[label] = y_vals.tolist()
        card_colors[label] = _month_color(m)
        fig.add_trace(
            go.Scatter(
                ...
                name=label,
                line=dict(color=_month_color(m), width=_line_width(), dash="solid"),
                hovertemplate="区分: %{fullData.name}<br>時刻: %{x}<br>最大: %{y:.2f} kW<extra></extra>",
            )
        )
    ...
    y_axis_mode = st.radio(
        ...
        ["自動", "手動"],
        horizontal=True,
        key="monthly_slot_peak_y_axis_mode",
    )
    manual_y_range: Optional[list[float]] = None
    if y_axis_mode == "手動":
        observed_y_max = max(
        ...
        with y_min_col:
            y_axis_min = st.number_input(
                ...
                value=0.0,
                step=1.0,
                key="monthly_slot_peak_y_axis_min_kw",
            )
        with y_max_col:
            y_axis_max = st.number_input(
                "Y軸の最大値 (kW)",
                value=suggested_y_max,
                step=1.0,
                key="monthly_slot_peak_y_axis_max_kw",
            )
        if y_axis_min < y_axis_max:
            manual_y_range = [float(y_axis_min), float(y_axis_max)]
        else:
    ...
    with title_size_col:
        y_axis_title_scale_percent = st.slider(
            ...
            value=100,
            step=10,
            key="monthly_slot_peak_y_axis_title_scale",
        )
    with tick_size_col:
        y_axis_tick_scale_percent = st.slider(
            "Y軸の数字の大きさ (%)",
            ...
            value=100,
            step=10,
            key="monthly_slot_peak_y_axis_tick_scale",
        )
    with legend_size_col:
        legend_scale_percent = st.slider(
            "凡例の大きさ (%)",
            ...
            value=100,
            step=10,
            key="monthly_slot_peak_legend_scale",
        )
    y_axis_title_scale = y_axis_title_scale_percent / 100.0
    y_axis_tick_scale = y_axis_tick_scale_percent / 100.0
    legend_scale = legend_scale_percent / 100.0
    ...
    csv_wide = (
        peak_curve.pivot_table(index="slot_label", columns="month", values="max_kw", aggfunc="max")
        .rename(columns=lambda x: f"{int(x)}月")
        .reindex(index=slot_order["slot_label"].tolist())
    )
    csv_out = csv_wide.reset_index().rename(columns={"slot_label": "時刻"})

    fallback = _fallback_multi_line_png(lines_for_png, "1～12月 時間帯別最大デマンド", "kW")
    card_png = _shadow_card_monthly_average_png(
        slot_labels=slot_order["slot_label"].astype(str).tolist(),
        series=lines_for_png,
        colors=card_colors,
        display_unit="kW",
        y_axis_label="最大デマンド[kW]",
        y_range=manual_y_range,
        y_axis_title_scale=y_axis_title_scale,
        y_axis_tick_scale=y_axis_tick_scale,
        legend_scale=legend_scale,
    ...
    with d2:
        st.download_button(
            "影付きグラフPNGダウンロード",
            data=card_png,
            file_name="各月時間帯別最大デマンドグラフ_影付き.png",
            mime="image/png",
            key="png_monthly_slot_peak_shadow_card",
        )
    with d3:
        _download_csv(
            csv_out,
...
def render_material_data(result: AnalysisResult) -> None:
    """スクリーンショットと同等の「資料作成用データ」。
    - 右端の相関係数は表内で縦結合（rowspan）表示
    - 表フォントサイズ / 太字の調整
    - 表PNG / CSVダウンロード
    """
    import html as _html

    st.subheader("資料作成用データ")

    # ユーザー調整（このセクション限定）
    c_ctrl1, c_ctrl2, c_ctrl3, c_ctrl4 = st.columns([1.2, 1.0, 1.2, 2.0])
    with c_ctrl1:
        font_size = st.slider("フォントサイズ", min_value=8, max_value=22, value=11, step=1, key="mat_font_size")
    with c_ctrl2:
        bold = st.checkbox("太字", value=False, key="mat_bold")
    with c_ctrl3:
        scale_pct = st.slider("表示倍率(%)", min_value=60, max_value=100, value=85, step=1, key="mat_scale_pct")
    scale = float(scale_pct) / 100.0
    with c_ctrl4:
        st.caption("※ 他の機能・表示には影響しません（資料作成用データのみ）。")
    ...
    # スクリーンショット順: 日, 月, 火, 水, 木, 金, 土
    order = ["日", "月", "火", "水", "木", "金", "土"]
    table["曜日"] = table["曜日"].astype(str)
    table = table.set_index("曜日").reindex(order).reset_index()

    # 相関（合計行を除外）
    corr_base = table[["最高デマンド値(kW)", "全時間使用量(kWh)", "日中使用量(kWh)"]].apply(pd.to_numeric, errors="coerce").dropna()
    corr1 = float(corr_base["最高デマンド値(kW)"].corr(corr_base["全時間使用量(kWh)"])) if len(corr_base) >= 2 else np.nan
    corr2 = float(corr_base["最高デマンド値(kW)"].corr(corr_base["日中使用量(kWh)"])) if len(corr_base) >= 2 else np.nan

    # 合計（色付け・統計・相関の対象外）
    sum_label = ""  # 画面では空欄（None禁止）
    full_sum = float(pd.to_numeric(table["全時間使用量(kWh)"], errors="coerce").sum())
    day_sum = float(pd.to_numeric(table["日中使用量(kWh)"], errors="coerce").sum())
    util = (day_sum / full_sum * 100.0) if full_sum > 0 else np.nan
    ...
    # 合計行を追加（最高/最低/平均は空欄、色付け対象外）
    sum_row = {
        col_weekday: sum_label,
        col_peak: np.nan,
        col_min: np.nan,
        col_avg: np.nan,
        col_full: full_sum,
    ...
    view = pd.concat([view, pd.DataFrame([sum_row])], ignore_index=True)

    # 色付け対象列（合計行は除外）
    heat_cols = [col_peak, col_full, col_day]
    base = view[view[col_weekday] != sum_label].copy()
    stats: dict[str, tuple[float, float, float]] = {}
    for c in heat_cols:
    ...
    def _bg_style(col_name: str, v: object, is_sum: bool) -> str:
        if is_sum:
            return "background-color: #FFFFFF; color: #000000;"
        if col_name not in heat_cols:
            return "background-color: #FFFFFF; color: #000000;"
        try:
            fv = float(v)
        except Exception:
        ...
        if not np.isfinite(fv):
            return "background-color: #FFFFFF; color: #000000;"
        vmin, vcenter, vmax = stats[col_name]
        return _cell_style_bg_only(fv, vmin, vcenter, vmax)

    corr1_txt = f"{corr1:.3f}" if np.isfinite(corr1) else "算出不可"
    corr2_txt = f"{corr2:.3f}" if np.isfinite(corr2) else "算出不可"

    # --- HTML table (rowspan for correlation columns) ---
    fw = "700" if bold else "400"
    header_bg = "#F2F2F2"
    border = "1px solid #C0C0C0"

    # Header
    headers = [col_weekday, col_peak, col_min, col_avg, col_full, col_day, col_rate, col_corr1, col_corr2]

    # Build rows (weekday rows 7, then sum row)
    rows_html = []
    weekday_rows = view[view[col_weekday] != sum_label].copy()
    sum_rows = view[view[col_weekday] == sum_label].copy()

    # 7 weekday rows (+ sum row = 8 rows) => correlation cells should span ALL rows (including sum row)
    for ridx in range(len(weekday_rows)):
        r = weekday_rows.iloc[ridx]
        tds = []
        # weekday
        tds.append(f'<td style="border:{border}; padding:6px; text-align:center; font-weight:{fw};">{_html.escape(str(r[col_weekday]))}</td>')
        # peak (heat)
        tds.append(f'<td style="border:{border}; padding:6px; text-align:right; {_bg_style(col_peak, r[col_peak], False)}">{_fmt_num(r[col_peak])}</td>')
        # min (no heat)
        tds.append(f'<td style="border:{border}; padding:6px; text-align:right;">{_fmt_num(r[col_min])}</td>')
        # avg (no heat)
        # avg in screenshot shows one decimal? existing table shows 15.4 etc; keep 1 decimal
        try:
            av = float(r[col_avg])
            av_txt = "" if not np.isfinite(av) else f"{av:.1f}"
        except Exception:
            av_txt = ""
        tds.append(f'<td style="border:{border}; padding:6px; text-align:right;">{av_txt}</td>')
        # full (heat)
        tds.append(f'<td style="border:{border}; padding:6px; text-align:right; {_bg_style(col_full, r[col_full], False)}">{_fmt_num(r[col_full])}</td>')
        # day (heat)
        tds.append(f'<td style="border:{border}; padding:6px; text-align:right; {_bg_style(col_day, r[col_day], False)}">{_fmt_num(r[col_day])}</td>')
        # rate (no heat)
        tds.append(f'<td style="border:{border}; padding:6px; text-align:right;">{_fmt_rate(r[col_rate])}</td>')

        # correlation columns: only in first weekday row with rowspan=8 (include sum row)
        if ridx == 0:
            tds.append(
                f'<td rowspan="8" style="border:{border}; padding:6px; text-align:center; vertical-align:middle; font-weight:{fw};">{_html.escape(corr1_txt)}</td>'
            )
            tds.append(
                f'<td rowspan="8" style="border:{border}; padding:6px; text-align:center; vertical-align:middle; font-weight:{fw};">{_html.escape(corr2_txt)}</td>'
            )

        rows_html.append("<tr>" + "".join(tds) + "</tr>")

    # sum row (no correlation values, no heat for any). Note: correlation cells are merged above.
    if not sum_rows.empty:
        r = sum_rows.iloc[0]
        tds = []
        tds.append(f'<td style="border:{border}; padding:6px; text-align:center; font-weight:{fw};"></td>')
        # peak/min/avg blank
        tds.append(f'<td style="border:{border}; padding:6px; text-align:right;"></td>')
        tds.append(f'<td style="border:{border}; padding:6px; text-align:right;"></td>')
        tds.append(f'<td style="border:{border}; padding:6px; text-align:right;"></td>')
        # sums shown but NO heat
        tds.append(f'<td style="border:{border}; padding:6px; text-align:right;">{_fmt_num(r[col_full])}</td>')
        tds.append(f'<td style="border:{border}; padding:6px; text-align:right;">{_fmt_num(r[col_day])}</td>')
        tds.append(f'<td style="border:{border}; padding:6px; text-align:right;">{_fmt_rate(r[col_rate])}</td>')
        # corr columns are merged (rowspan) so we do NOT add cells here
        rows_html.append("<tr>" + "".join(tds) + "</tr>")

    header_html = "".join([f'<th style="border:{border}; padding:6px; background:{header_bg}; text-align:center; font-weight:{fw};">{_html.escape(h).replace(" ", "<br>")}</th>' for h in headers])

    table_html = f'''
    <div style="overflow-x:auto;">
      <div style="transform: scale({scale}); transform-origin: top left; width: calc(100% / {scale});">
        <table style="border-collapse:collapse; width:100%; font-size:{font_size}px; font-weight:{fw};">
    ...
    # --- PNG / CSV ---
    # PNGは「画面に見えている表」と同じ見た目で出力する（別フォーマットのヒートマップは使わない）

    def _material_table_png_same_as_view() -> bytes:
        """Render the same table as the on-screen HTML into a PNG.
        - Correlation cells are vertically merged across ALL rows (including sum row).
        - Uses the same background colors as the HTML (heat columns only, sum row excluded).
        """
        from PIL import Image, ImageDraw

        # Column widths tuned to match the HTML table visually.
        col_defs = [
            (col_weekday, 70, "center"),
            (col_peak, 140, "right"),
            (col_min, 140, "right"),
            (col_avg, 140, "right"),
            (col_full, 170, "right"),
            (col_day, 200, "right"),
            (col_rate, 170, "right"),
            (col_corr1, 120, "center"),
            (col_corr2, 120, "center"),
        ]

        # Apply compact scale (資料作成用データ PNG 出力のみ)
        _scale = float(scale) if isinstance(scale, (int, float)) else 1.0

        def _sw(v: int, min_v: int) -> int:
            return max(int(min_v), int(round(float(v) * _scale)))

        col_defs = [(n, _sw(int(w), 60 if n == col_weekday else 90), a) for (n, w, a) in col_defs]

        header_h = _sw(42, 28)
        row_h = _sw(30, 22)
        pad_x = _sw(8, 4)

        font = _load_jp_font(max(8, int(round((font_size + 2) * _scale))))
        font_header = _load_jp_font(max(8, int(round((font_size + 1) * _scale))))
        border_col = (192, 192, 192)  # #C0C0C0
        header_bg_rgb = (242, 242, 242)  # #F2F2F2
        white_rgb = (255, 255, 255)

        n_rows = 8  # 7 weekdays + sum
        w = sum(w for _, w, _ in col_defs) + 2
        h = header_h + n_rows * row_h + 2
        img = Image.new("RGB", (w, h), white_rgb)
        draw = ImageDraw.Draw(img)

        # Header
        x = 0
        for name, cw, _align in col_defs:
            draw.rectangle((x, 0, x + cw, header_h), fill=header_bg_rgb, outline=border_col)
            label = str(name).replace(" ", "\n")
            draw.multiline_text((x + cw / 2, header_h / 2), label, fill=(0, 0, 0), anchor="mm", align="center", font=font_header)
            x += cw

        # Body rows: 7 weekdays + sum
        body_rows = pd.concat([weekday_rows, sum_rows], ignore_index=True)

        def _bg_rgb_for_cell(col_name: str, value: object, is_sum: bool) -> tuple[int, int, int]:
            if is_sum or col_name not in heat_cols:
                return white_rgb
            try:
                fv = float(value)
            except Exception:
            ...
            if not np.isfinite(fv):
                return white_rgb
            vmin, vcenter, vmax = stats.get(col_name, (0.0, 0.0, 0.0))
            if vmax <= vmin:
                return white_rgb
            style = _cell_style_bg_only(fv, vmin, vcenter, vmax)
            try:
                rgb_txt = style.split("rgb(", 1)[1].split(")", 1)[0]
                rr, gg, bb = [int(t.strip()) for t in rgb_txt.split(",")]
                return (rr, gg, bb)
            except Exception:
                return white_rgb

        # Draw per-row cells except merged correlation columns
        for ridx in range(n_rows):
            y0 = header_h + ridx * row_h
            y1 = y0 + row_h
            r = body_rows.iloc[ridx]
            is_sum = (ridx == n_rows - 1)

            x = 0
            for (cname, cw, align) in col_defs:
                if cname in (col_corr1, col_corr2):
                    x += cw
                    continue

                bg = _bg_rgb_for_cell(cname, r.get(cname, np.nan), is_sum)
                draw.rectangle((x, y0, x + cw, y1), fill=bg, outline=border_col)

                if cname == col_weekday:
                    txt = "" if is_sum else str(r.get(cname, ""))
                elif cname == col_rate:
                    txt = _fmt_rate(r.get(cname, np.nan))
                elif cname == col_avg:
                    try:
                        av = float(r.get(cname, np.nan))
                        txt = "" if (is_sum or not np.isfinite(av)) else f"{av:.1f}"
                    except Exception:
                        txt = ""
                else:
                    txt = _fmt_num(r.get(cname, np.nan))

                if txt:
                    if align == "center":
                        draw.text((x + cw / 2, y0 + row_h / 2), txt, fill=(0, 0, 0), anchor="mm", font=font)
                    elif align == "right":
                        draw.text((x + cw - pad_x, y0 + row_h / 2), txt, fill=(0, 0, 0), anchor="rm", font=font)
                    else:
                        draw.text((x + pad_x, y0 + row_h / 2), txt, fill=(0, 0, 0), anchor="lm", font=font)

                x += cw

        # Draw merged correlation cells spanning ALL rows
        x_corr1 = sum(w for name, w, _ in col_defs if name not in (col_corr1, col_corr2))
        w_corr1 = next(w for name, w, _ in col_defs if name == col_corr1)
        w_corr2 = next(w for name, w, _ in col_defs if name == col_corr2)
        x_corr2 = x_corr1 + w_corr1
        y_corr0 = header_h
        y_corr1 = header_h + n_rows * row_h

        draw.rectangle((x_corr1, y_corr0, x_corr1 + w_corr1, y_corr1), fill=white_rgb, outline=border_col)
        draw.text((x_corr1 + w_corr1 / 2, (y_corr0 + y_corr1) / 2), corr1_txt, fill=(0, 0, 0), anchor="mm", font=font)

        draw.rectangle((x_corr2, y_corr0, x_corr2 + w_corr2, y_corr1), fill=white_rgb, outline=border_col)
        draw.text((x_corr2 + w_corr2 / 2, (y_corr0 + y_corr1) / 2), corr2_txt, fill=(0, 0, 0), anchor="mm", font=font)

        buf = BytesIO()
        img.save(buf, format="PNG")
        return buf.getvalue()

    # CSV用に相関列も含めた表を作る（相関は先頭行にのみ入れる）
    csv_view = view.copy()
    csv_view[col_corr1] = ""
    csv_view[col_corr2] = ""
    if len(csv_view) >= 1:
        csv_view.loc[0, col_corr1] = corr1_txt
        csv_view.loc[0, col_corr2] = corr2_txt

    png = _material_table_png_same_as_view()

    d1, d2 = st.columns(2)
    with d1:
        st.download_button("表をPNGでダウンロード", data=png, file_name="資料作成用データ.png", mime="image/png", key="png_material_table")
    with d2:
        _download_csv(csv_view, "CSVダウンロード", "資料作成用データ.csv", "csv_material_table")

    # --- グラフ（色はスクリーンショットに合わせて固定） ---
    chart_df = weekday_rows.copy()
    fig = go.Figure()
    fig.add_trace(go.Bar(x=chart_df[col_weekday], y=pd.to_numeric(chart_df[col_full], errors="coerce"), name="全日", marker_color="#FFC000"))
    fig.add_trace(go.Bar(x=chart_df[col_weekday], y=pd.to_numeric(chart_df[col_day], errors="coerce"), name="日中", marker_color="#F6C6AD"))
    fig.add_trace(
        go.Scatter(
            x=chart_df[col_weekday],
            y=pd.to_numeric(chart_df[col_peak], errors="coerce"),
            name="デマンド値",
            mode="markers",
            marker=dict(color="#FF3399", size=8),
            yaxis="y2",
        )
    )
    fig.update_layout(
        title="全日＆日中(6:00-18:00)電力量",
        barmode="group",
        xaxis_title="曜日",
        # xグリッドはオフ、yグリッドは単一のみ（重なり対策）
        xaxis=dict(showgrid=False),
        yaxis=dict(title="kWh", rangemode="tozero", showgrid=True, gridcolor="#E6E6E6", gridwidth=1, tickformat=",.0f"),
        yaxis2=dict(title="kW", overlaying="y", side="right", rangemode="tozero", showgrid=False),
...
def render_selected_sections(
    ...
    show_material_data: bool,
    facility_name: str = "",
    start_date: Optional[date] = None,
    end_date: Optional[date] = None,
    show_monthly_slot_peak: bool = False,
) -> None:
    if show_summary:
    ...
    if show_avg_ext:
        render_annual_monthly_average(result, facility_name=facility_name, start_date=start_date, end_date=end_date)
    if show_monthly_slot_peak:
        render_monthly_slot_peak(result)
    if show_actual_ext:
        render_actual_curve(result)
    ...
    if show_material_data:
        render_material_data(result)
def render_actual_curve(result: AnalysisResult) -> None:
    st.subheader("グラフ（実データ）")

    df = result.target_data.copy()
    df = df.sort_values("datetime")

    df["日時"] = pd.to_datetime(df["datetime"]).dt.strftime("%Y-%m-%d %H:%M")
    df = df.rename(columns={"kw": "デマンド(kW)"})
