...
def render_material_data(result: AnalysisResult) -> None:
    ...
    # ユーザー調整（このセクション限定）
    c_ctrl1, c_ctrl2, c_ctrl3 = st.columns([1.2, 1.0, 2.0])
    with c_ctrl1:
    ...
    def _material_table_png_same_as_view() -> bytes:
        ...
        for name, cw, _align in col_defs:
            ...
            draw.multiline_text((x + cw / 2, header_h / 2), label, fill=(0, 0, 0), anchor="mm", align="center")
            x += cw
        ...
        for ridx in range(n_rows):
            ...
            for (cname, cw, align) in col_defs:
                ...
                if txt:
                    if align == "center":
                        draw.text((x + cw / 2, y0 + row_h / 2), txt, fill=(0, 0, 0), anchor="mm")
                    elif align == "right":
                        draw.text((x + cw - pad_x, y0 + row_h / 2), txt, fill=(0, 0, 0), anchor="rm")
                    else:
                        draw.text((x + pad_x, y0 + row_h / 2), txt, fill=(0, 0, 0), anchor="lm")
        ...
        draw.text((x_corr1 + w_corr1 / 2, (y_corr0 + y_corr1) / 2), corr1_txt, fill=(0, 0, 0), anchor="mm")

        draw.rectangle((x_corr2, y_corr0, x_corr2 + w_corr2, y_corr1), fill=white_rgb, outline=border_col)
        draw.text((x_corr2 + w_corr2 / 2, (y_corr0 + y_corr1) / 2), corr2_txt, fill=(0, 0, 0), anchor="mm")
