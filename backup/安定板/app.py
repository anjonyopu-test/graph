...
def main() -> None:
    ...
    with up_col2:
        ...
        input_unit = "kw" if unit_label.startswith("kW") else "kwh"
    ...
    with cb1:
        show_summary = st.checkbox("サマリー", value=True)
        show_input_heatmap = st.checkbox("入力データ可視化", value=False)
        show_sheet_like = st.checkbox("月毎データ", value=True)
    with cb2:
        show_avg_ext = st.checkbox("グラフ（平均値）", value=True)
        show_monthly_actual = st.checkbox("グラフ（月別実績）", value=True)
        show_weekday_daytime_graph = st.checkbox("曜日別日中デマンド", value=False)
        show_month_daytime_graph = st.checkbox("月別日中デマンド", value=True)
    with cb3:
        show_std = st.checkbox("月別・曜日別 標準偏差", value=True)
        show_range = st.checkbox("デマンドレンジ", value=True)
    with cb4:
        show_peak_cut = st.checkbox("ピークカット目安試算", value=True)
        show_weekday_curve = st.checkbox("曜日別平均負荷曲線", value=False)
        show_slot_max_count = st.checkbox("時間別デマンド回数（時間別最高デマンド値）", value=True)
